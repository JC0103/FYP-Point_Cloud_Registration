...
// For callibration
const int L_zeros_angle = 0;
const int R_zeros_angle = 0;
...
void loop() {
  ...
  if (abs(millis() - previous_write_time) > 600){
    ...
    L_servo.write(angle);                  // sets the servo position according to the scaled value
  }                      
  nh.spinOnce();
  delay(3);
}
...
void servoAngleCB(const std_msgs::Int16& msg){
    ...
    (msg.data + R_zeros_angle < min_angle && msg.data + R_zeros_angle > max_angle)){
